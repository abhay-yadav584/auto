...
# NOTE: Classes here delegate parsing to NetworkParsers; no parsing regex duplicated.
    ...
    def _extract_block(self, markers):
        ...
        for i, l in enumerate(lines):
            ...
            if any(m in lower for m in markers):
                start = i + 1
                continue
            if start is not None and "#sh " in lower and not any(m in lower for m in markers):
...
class BgpStatus:
    ...
    def _bgp_lines(self):
        ...
        for i, l in enumerate(lines):
            lower = l.lower()
            if "#sh bgp summary" in lower or "show bgp summary" in lower:
                start = i + 1
                continue
            if start is not None and "#sh " in lower and "bgp summary" not in lower:
                return [x for x in lines[start:i] if x.strip()]
        if start is None:
            return []
        return [x for x in lines[start:] if x.strip()]
    ...
    def _records(self):
        ...
        for line in self._bgp_lines():
            low = line.lower()
            if low.startswith("neighbor") or line.startswith("-"):
                continue
            parts = re.split(r'\s{2,}', line.strip())
    ...
    def count_established_sessions(self):
        return sum(1 for r in self._records() if r[2].lower() == "established")
...
class RouteSummary:
    def __init__(self, raw: str):
        self.raw = raw
        self.parser = NetworkParsers()

    def rows(self):
        return self.parser.parse_ip_route_summary(self.raw)
    ...
    def print(self):
        ...
        name_width = max(len(r["SOURCE"]) for r in counted_rows) if counted_rows else len("SOURCE")
        ...
        for r in rows:
            ...
            if cnt is None:
                ...
                print(f"{r.get('SOURCE','').ljust(target_width)}{str(cnt).rjust(4)}")
        print("")
...
class IgmpSnoopingQuerier:
    ...
    def data(self):
        return self.parser.parse_igmp_snooping_querier(self.raw)
...
class VlanBrief:
    ...
    def rows(self):
        return self.parser.parse_vlan_brief(self.raw)
...
class VlanDynamic:
    ...
    def rows(self):
        return self.parser.parse_vlan_dynamic(self.raw)
...
class EvpnRouteTypes:
    ...
    def auto_discovery(self):
        return self.parser.parse_bgp_evpn_route_type_auto_discovery(self.raw)

    def mac_ip(self):
        return self.parser.parse_bgp_evpn_route_type_mac_ip(self.raw)

    def imet(self):
        return self.parser.parse_bgp_evpn_route_type_imet(self.raw)
    ...
    def print_summary(self):
        ...
        ad = self.auto_discovery()
        mac = self.mac_ip()
        imet = self.imet()
        eth = self.ethernet_segment()
        print(f"auto-discovery: {len(ad)} entries")
        print(f"mac-ip: {len(mac)} entries")
        print(f"imet: {len(imet)} entries")
        if eth:
            print(f"ethernet-segment: {len(eth)} entries")
